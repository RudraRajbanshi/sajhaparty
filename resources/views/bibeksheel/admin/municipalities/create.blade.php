    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.municipalities.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('wards') ? 'is-invalid' : '' }}" type="number" name="wards" id="wards" value="{{ old('wards') }}" step="1" required>
                @if($errors->has(''))
                    <div class="invalid-feedback">
                        {{ $errors->first('') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('municipality_code') ? 'is-invalid' : '' }}" type="text" name="municipality_code" id="municipality_code" value="{{ old('municipality_code', '') }}" required>
                @if($errors->has(''))
                    <div class="invalid-feedback">
                        {{ $errors->first('') }}
                    </div>
