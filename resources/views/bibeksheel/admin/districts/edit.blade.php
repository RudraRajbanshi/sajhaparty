        ...
        <form method="POST" action="{{ route("admin.districts.update", [$district->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
             ...
             <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('district_code') ? 'is-invalid' : '' }}" type="text" name="district_code" id="district_code" value="{{ old('district_code', $district->district_code) }}" required>
                @if($errors->has(''))
                    <div class="invalid-feedback">
                        {{ $errors->first('') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\District::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $district->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has(''))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select data-toggle="select" class="form-control select2 {{ $errors->has('province') ? 'is-invalid' : '' }}" name="province_id" id="province_id" required>
                    @foreach($provinces as $id => $province)
                        <option value="{{ $id }}" {{ ($district->province ? $district->province->id : old('province_id')) == $id ? 'selected' : '' }}>{{ $province }}</option>
                    @endforeach
                </select>
                @if($errors->has(''))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('remarks') ? 'is-invalid' : '' }}" type="text" name="remarks" id="remarks" value="{{ old('remarks', $district->remarks) }}">
                @if($errors->has(''))
                    <div class="invalid-feedback">
                        {{ $errors->first('') }}
                    </div>
